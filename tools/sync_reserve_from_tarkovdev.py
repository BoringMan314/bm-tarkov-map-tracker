#!/usr/bin/env python3
"""Refresh reserve DEV A satellite PNG + meta (z=4, transparent crop + black flatten)."""
...
MAPS_DIR = ROOT / "internal" / "maps_tarkov.dev" / "reserve"
MAPS_B_DIR = ROOT / "internal" / "maps_tarkov.dev_B" / "reserve"
...
TILE_URL = "https://assets.tarkov.dev/maps/reserve/main/{z}/{x}/{y}.png"
TILE_ZOOM = 4
TILE_SIZE = 256
MIN_TILE_BYTES = 4000
SCAN = 24
...
def map_variant(maps_data: list) -> dict:
    for entry in maps_data:
        if entry.get("normalizedName") != "reserve":
            continue
        for variant in entry.get("maps") or []:
            if variant.get("key") == "reserve" and variant.get("projection") == "interactive":
                return variant
    raise SystemExit("FAIL: reserve variant not found")
...
def filter_main_cluster(coords: list[tuple[int, int]]) -> list[tuple[int, int]]:
    if TILE_ZOOM >= 6:
        cluster = [(x, y) for x, y in coords if x <= 39 and y <= 39]
        if len(cluster) >= 400:
            return cluster
    if TILE_ZOOM == 5:
        cluster = [(x, y) for x, y in coords if x <= 31 and y <= 29]
        if len(cluster) >= 400:
            return cluster
    if TILE_ZOOM == 4:
        cluster = [(x, y) for x, y in coords if x <= 15 and y <= 14]
        if len(cluster) >= 100:
            return cluster
    return coords
    ...
    with ThreadPoolExecutor(max_workers=16) as pool:
        for hit in pool.map(lambda xy: probe_at(*xy), jobs):
            if hit:
                coords.append(hit)
    if not coords:
        raise SystemExit("FAIL: no reserve tiles found")
...
def download_and_stitch(
    ...
    crop = canvas.getbbox()
    if not crop:
        raise SystemExit("FAIL: stitched reserve map is empty")
...
def patch_dev_b_tile_meta(min_x: int, max_x: int, min_y: int, max_y: int) -> None:
    ...
    print("OK dev_B reserve/meta.json tile reference updated")
...
def main() -> None:
    ...
    print(f"OK reserve z={TILE_ZOOM} bbox=({min_x},{min_y})-({max_x},{max_y})")
    ...
    print(f"OK reserve/map.png {crop_w}x{crop_h} ({len(buf.getvalue())/1024/1024:.1f} MB)")
