#!/usr/bin/env python3
"""Verify shoreline PMC extract pixels land on DEV A satellite PNG."""
...
META = json.loads((ROOT / "internal/maps_tarkov.dev/shoreline/meta.json").read_text(encoding="utf-8"))
PMC = json.loads((ROOT / "internal/points/exfil/pmc.json").read_text(encoding="utf-8"))["shoreline"]
...
def game_to_map_pixels(game_x: float, game_z: float, meta: dict) -> tuple[float, float]:
    ...
    layer = (layer[0] * factor, layer[1] * factor)
    bounds = {
        "minX": tmin_x * tile,
        "minY": tmin_y * tile,
        "maxX": (tmax_x + 1) * tile,
...
def main() -> None:
    png_path = ROOT / "internal/maps_tarkov.dev/shoreline/map.png"
    ...
    out = ROOT / "tools/_shoreline_verify.png"
    img.save(out)
