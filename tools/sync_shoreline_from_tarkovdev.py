#!/usr/bin/env python3
"""Refresh shoreline DEV A satellite PNG + meta (z=4, transparent crop + black flatten)."""
...
MAPS_DIR = ROOT / "internal" / "maps_tarkov.dev" / "shoreline"
MAPS_B_DIR = ROOT / "internal" / "maps_tarkov.dev_B" / "shoreline"
...
def map_variant(maps_data: list) -> dict:
    for entry in maps_data:
        if entry.get("normalizedName") != "shoreline":
            continue
        for variant in entry.get("maps") or []:
            if variant.get("key") == "shoreline" and variant.get("projection") == "interactive":
                return variant
    raise SystemExit("FAIL: shoreline variant not found")
...
def filter_main_cluster(coords: list[tuple[int, int]]) -> list[tuple[int, int]]:
    """Drop sparse fringe tiles that expand bbox with empty cells."""
    if TILE_ZOOM >= 6:
        cluster = [(x, y) for x, y in coords if x <= 31 and y <= 31]
        if len(cluster) >= 400:
            return cluster
    if TILE_ZOOM == 4:
        cluster = [(x, y) for x, y in coords if x <= 15 and y <= 13]
        if len(cluster) >= 100:
            return cluster
    return coords
    ...
    with ThreadPoolExecutor(max_workers=16) as pool:
        for hit in pool.map(lambda xy: probe_at(*xy), jobs):
            if hit:
                coords.append(hit)
    if not coords:
        raise SystemExit("FAIL: no shoreline tiles found")
...
def download_and_stitch(
    ...
    crop = canvas.getbbox()
    if not crop:
        raise SystemExit("FAIL: stitched shoreline map is empty")
...
def patch_dev_b_tile_meta(min_x: int, max_x: int, min_y: int, max_y: int) -> None:
    ...
    print("OK dev_B shoreline/meta.json tile reference updated")
...
def main() -> None:
    ...
    print(f"OK shoreline z={TILE_ZOOM} bbox=({min_x},{min_y})-({max_x},{max_y})")
    ...
    print(f"OK shoreline/map.png {crop_w}x{crop_h} ({len(buf.getvalue())/1024/1024:.1f} MB)")
