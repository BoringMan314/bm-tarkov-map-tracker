#!/usr/bin/env python3
"""Plot lighthouse PMC extract pixels on map PNG (transform mapping, no tile_zoom)."""
...
META = json.loads((ROOT / "internal/maps_tarkov.dev/lighthouse/meta.json").read_text(encoding="utf-8"))
PMC = json.loads((ROOT / "internal/points/exfil/pmc.json").read_text(encoding="utf-8"))["lighthouse"]
...
def game_to_map_pixels(game_x: float, game_z: float, meta: dict) -> tuple[float, float]:
    ...
    # exfil marker rotation 180 (default)
...
def main() -> None:
    png_path = ROOT / "internal/maps_tarkov.dev/lighthouse/map.png"
    ...
    out = ROOT / "tools/_lighthouse_verify.png"
    img.save(out)
